...
def test_list_facts(mock_db) -> None:
    ...
    expected = [
        call.notice(
            "?aa, ?ab, ?ac, ?ad, ?ae, ?af, ?ag, ?ah, ?ai, ?aj, ?ak, ?al, ?am, ?an, ?ao, ?ap, ?aq, ?ar, ?as, ?at, ?au, ?av, ?aw, ?ax, ?ay, ?az, ?ba, ?bb, ?bc, ?bd, ?be, ?bf, ?bg, ?bh, ?bi, ?bj, ?bk, ?bl, ?bm, ?bn, ?bo, ?bp, ?bq, ?br, ?bs, ?bt, ?bu, ?bv, ?bw, ?bx, ?by, ?bz, ?ca, ?cb, ?cc, ?cd, ?ce, ?cf, ?cg, ?ch, ?ci, ?cj, ?ck, ?cl, ?cm, ?cn, ?co, ?commands, ?cp, ?cq, ?cr, ?cs, ?ct, ?cu, ?cv, ?cw, ?cx, ?cy"  # codespell:ignore bu
        ),
        call.notice(
            "?cz, ?da, ?db, ?dc, ?dd, ?de, ?df, ?dg, ?dh, ?di, ?dj, ?dk, ?dl, ?dm, ?dn, ?do, ?dp, ?dq, ?dr, ?ds, ?dt, ?du, ?dv, ?dw, ?dx, ?dy, ?dz, ?ea, ?eb, ?ec, ?ed, ?ee, ?ef, ?eg, ?eh, ?ei, ?ej, ?ek, ?el, ?em, ?en, ?eo, ?ep, ?eq, ?er, ?es, ?et, ?eu, ?ev, ?ew, ?ex, ?ey, ?ez, ?fa, ?fb, ?fc, ?fd, ?fe, ?ff, ?fg, ?fh, ?fi, ?fj, ?fk, ?fl, ?fm, ?fn, ?fo, ?fp, ?fq, ?fr, ?fs, ?ft, ?fu, ?fv, ?fw, ?fx, ?fy, ?fz, ?ga"  # codespell:ignore fo
        ),
        call.notice(
        ...
        call.notice(
            "?mf, ?mg, ?mh, ?mi, ?mj, ?mk, ?ml, ?mm, ?mn, ?mo, ?mp, ?mq, ?mr, ?ms, ?mt, ?mu, ?mv, ?mw, ?mx, ?my, ?mz, ?na, ?nb, ?nc, ?nd, ?ne, ?nf, ?ng, ?nh, ?ni, ?nj, ?nk, ?nl, ?nm, ?nn, ?no, ?np, ?nq, ?nr, ?ns, ?nt, ?nu, ?nv, ?nw, ?nx, ?ny, ?nz, ?oa, ?ob, ?oc, ?od, ?oe, ?of, ?og, ?oh, ?oi, ?oj, ?ok, ?ol, ?om, ?on, ?oo, ?op, ?oq, ?or, ?os, ?ot, ?ou, ?ov, ?ow, ?ox, ?oy, ?oz, ?pa, ?pb, ?pc, ?pd, ?pe, ?pf, ?pg"  # codespell:ignore nd,ot
        ),
        call.notice(
        ...
        call.notice(
            "?sj, ?sk, ?sl, ?sm, ?sn, ?so, ?sp, ?sq, ?sr, ?ss, ?st, ?su, ?sv, ?sw, ?sx, ?sy, ?sz, ?ta, ?tb, ?tc, ?td, ?te, ?tf, ?tg, ?th, ?ti, ?tj, ?tk, ?tl, ?tm, ?tn, ?to, ?tp, ?tq, ?tr, ?ts, ?tt, ?tu, ?tv, ?tw, ?tx, ?ty, ?tz, ?ua, ?ub, ?uc, ?ud, ?ue, ?uf, ?ug, ?uh, ?ui, ?uj, ?uk, ?ul, ?um, ?un, ?uo, ?up, ?uq, ?ur, ?us, ?ut, ?uu, ?uv, ?uw, ?ux, ?uy, ?uz, ?va, ?vb, ?vc, ?vd, ?ve, ?vf, ?vg, ?vh, ?vi, ?vj, ?vk"  # codespell:ignore te,ue
        ),
        call.notice(
    ...
    assert event.mock_calls == expected
